...
pub struct Tree<I: Index, V: Value> {
  ...
  root: Arc<Mutex<Box<Node<I, V>>>>,               // TYPE : 트리의 루트 노드
...
impl<I: Index, V: Value> Tree<I,V> {
  pub fn new(base_path: PathBuf, node_name: &str) -> Self {
    ...
    Self {
      ...
      root: Arc::new(Mutex::new(Box::new(root))),
  ...
  pub fn insert(&mut self, index: I, value: V) -> Result<(), String> {
    let mut node_file = self.node_file.lock().unwrap();
    {
      ...
      // debug!("MIDDLE INSERT(i:{:?}) {:#?}", index, temp_root);
      // temp_root.write(&mut node_file).unwrap();
    }

    let temp_root = Node::<I, V>::read(&mut node_file, 0).unwrap();
    debug!("AFTER INSERT {:#?}", temp_root);
    self.root = Arc::new(Mutex::new(Box::new(temp_root)));
